...
function getConnection(ID) {  //Find a specific connection from the database and return it
    return new Promise((resolve, reject) => {
        connectionModel.findOne({ _id: ID })
            .then((data) => {
                if (data == null) {     //if connection isn't found, return -1
                    resolve(-1);
                }
                else {
                    let connectionObj = new Connection(data._id, data.type, data.game, data.details, data.time, data.location, data.userID);
